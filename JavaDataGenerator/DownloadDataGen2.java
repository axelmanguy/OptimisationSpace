...
public class DownloadDataGen2 {
	...
	public static void writeDatFile(SolutionPlan plan,Satellite satellite, String datFilename) throws IOException{
		...
		//write the list of volume of the acquisitions Params.downlinkRate
		writer.write("\nSizeAcq = [");
		if(!candidateDownloads.isEmpty()){
			writer.write(""+(int)Math.ceil(candidateDownloads.get(0).getVolume()));
			for(int i=1;i<Nacq;i++){
				writer.write(","+(int)Math.ceil(candidateDownloads.get(i).getVolume()));
			}
		}
		writer.write("];");
		...
		writer.write("\nDuration = [");
		if(!candidateDownloads.isEmpty()){
			writer.write(""+(int)Math.ceil((candidateDownloads.get(0).getVolume()/Params.downlinkRate)));
			for(int i=1;i<Nacq;i++){
				writer.write(","+(int)Math.ceil(candidateDownloads.get(i).getVolume()/Params.downlinkRate));
			}
		}
		writer.write("];");
		...
		writer.write("\nDateAcq = [");
		if(!candidateDownloads.isEmpty()){
			writer.write(""+(int)Math.ceil(candidateDownloads.get(0).getAcquisitionTime()));
			for(int i=1;i<Nacq;i++){
				writer.write(","+(int)Math.ceil(candidateDownloads.get(i).getAcquisitionTime()));
			}
		}
		writer.write("];");
		...
		if(!pb.downloadWindows.isEmpty()){
			if(pb.downloadWindows.get(0).satellite==satellite){
				writer.write(""+(int)pb.downloadWindows.get(0).start);
			}
			for(int i=1;i<pb.downloadWindows.size();i++){
				if(pb.downloadWindows.get(i).satellite==satellite) {
					writer.write(","+(int)pb.downloadWindows.get(i).start);
				}
			}
		}
		writer.write("];");
		...
		if(!pb.downloadWindows.isEmpty()){
			if(pb.downloadWindows.get(0).satellite==satellite){
				writer.write(""+(int)pb.downloadWindows.get(0).end);
			}
			for(int i=1;i<pb.downloadWindows.size();i++){
				if(pb.downloadWindows.get(i).satellite==satellite) {
					writer.write(","+(int)pb.downloadWindows.get(i).end);
				}
			}
		}
		writer.write("];");
	...
	public static void main(String[] args) throws XMLStreamException, FactoryConfigurationError, IOException{
		...
		for(Satellite satellite : pb.satellites){
			String datFilename = "output/INTDownloadDATA_"+satellite.name+".dat";
